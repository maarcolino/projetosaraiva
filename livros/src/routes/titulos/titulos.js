...
route_titulo.get("/detalhes",(req,res)=>{
    data.query("select t.idtitulo,t.nometitulo,t.autor,t.sinopse,t.datacadastro,p.precoatual,p.precodesconto,f.foto1,f.foto2,f.foto3,f.foto4 from precos p inner join titulos t on p.idpreco = t.idprecos inner join fotos f on t.idfotos - f.idfotos",(error,result)=>{
    if(error){
        return res.status(500).send({msg:"Erro ao carregar os titulos"})
    }
    })
...
route_titulo.post("/cadastrar",(req,res)=>{
    data.query("insert into titulos set ?",req,body,(error,result)=>{
        if(error){
            res.status(500).send({msg:"Erro ao cadastrar o título"});
        }
